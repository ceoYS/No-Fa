/*
 * EmberCat — inline SVG silhouette placeholder.
 * Real cat sprites are NOT committed; CSS gradient + SVG silhouette is the P0 stand-in.
 * Direction: soft cream/white kitten (PRD §0.5, PET_CUSTOMIZATION_SPEC §10).
 * Expected (post-approval) asset paths: /public/pets/{white,black,gray,brown}.webp
 * Variant `tone`:
...
export default function EmberCat({ tone = 'steady', label }) {
  const stageClass =
    tone === 'dim' ? 'pet-stage pet-stage--dim'
      : tone === 'bright' ? 'pet-stage pet-stage--bright'
        : 'pet-stage';
...
function CatSilhouette() {
  return (
    ...
    >
      ...
      {/* sitting kitten silhouette — soft cream */}
      <path
        fill="url(#catBody)"
      ...
      />
      {/* ears — cream outer with warm inner */}
