...
| H2 | P1 | Home was a flat 7-card stack (timer, 규율, 무너졌다면, 멈춤, 최근 기록, 동행, 고양이 방); the core crisis loop was buried mid-scroll and the 동행 card was generic filler. | FIXED — timer-first refocus: abstinence timer hero + `지금 충동 멈추기`/`오늘 상태 남기기` hero CTAs; standalone 멈춤 card folded into the hero, generic 동행 card removed, 규율 card stripped of its duplicate check-in button. Hero + 4 compact cards. Pinned by `check:nof` (home timer-first). |
...
| C6 | P1 | Purpose was unclear — `1분 체크인` / `오늘 어땠어요?` read like the app's main mechanic rather than secondary personal logging. | FIXED — reframed to `1분 기록` / `오늘 상태를 남겨요` with purpose copy `패턴을 보기 위한 개인 기록이에요. 밖으로 공유되지 않아요.` The core loop is now the timer + crisis flow (Home hero); check-in is positioned as the secondary `오늘 상태 남기기` CTA. Persistence + once-per-day reward guard unchanged. |
...
| U2 | P0 | Misleading navigation: `대체 활동 해보기` routed straight to 홈 under a label that promises an alternative activity — a reviewer taps it expecting a coping action and lands on the home scroll. | FIXED — now opens a real in-screen alternative-action panel (`view='alt'`) with 4 concrete actions (물 한 잔 마시기 / 휴대폰 내려놓기 / 10번 천천히 숨쉬기 / 자리에서 일어나기). Completing one returns to the breath timer with a calm note; grants **no** shards (only once-per-day `crisisHeld`). Pinned by `check:nof` (urge alt-panel). |
| U3 | P1 | Forbidden copy: `파도처럼 약해질 수 있어요` appeared on both Home (멈춤 card) and Urge (subtitle) — an over-promise the app can't show. | FIXED — replaced with `5분만 지나도 강도는 조금 내려갈 수 있어요` / `지금은 선택을 늦추는 시간이에요`. `파도처럼` added to the `check:nof` forbidden-token scan so it can't return. |
...
| P1h | P1 | Snack feed felt weak — only a text line changed, no sense of handing the snack over. | FIXED — added an emoji-free ember hand-off token (`snack-toss-token`) that rises from the 간식 주기 button toward the scene on a successful feed, plus honest copy `간식을 건넸어요. 고양이가 기분 좋아 보여요.` No 먹었어요/eating/motion claim; the token is a delivery cue only and is suppressed under `prefers-reduced-motion`. Drag stays disabled. Pinned by `check:nof` (feed hand-off). |
...
| X5 | P1 | Source-level regression guard (`npm run check:nof` → `scripts/nof-regression-check.mjs`): pins the commercial invariants a passing build won't catch — reward claim guard present in App **and** screen, `isMilestoneClaimable` gating, check-in once-per-day grant, no discipline delete affordance, scene-mode drag disabled until `spriteReady`, no fake-motion/emoji-furniture/blob tokens (now incl. `파도처럼`), plus crisis-held once-per-day, slip-reflection once-per-day, pet-room sheets are dialogs, a top-level ErrorBoundary, home timer-first (hero CTAs), the urge alternative-action panel, and the honest snack hand-off. | EXPANDED — 13/13 checks pass; dependency-free node script wired as an npm script for a pre-review run. |
