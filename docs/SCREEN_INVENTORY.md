...
- 표시 데이터: 절제 경과 시간(일·시:분:초), 현재/최장 스트릭, character_stage, 오늘 체크인 여부, 오늘 보호한 자극 count(이미지/영상 mock count), discipline 상태(enum: 지키는 중 / 이번 주 실천 / 오늘 확인 필요), room_warmth 상태(enum: 약함 / 잔잔함 / 안정 / 따뜻함 — 내부 계산값은 비공개)
- 금지 카피: "참아라", 명령형, 내부 정책 어휘(예: "중립 표기·처벌 없음", "코스메틱·케어 기반 보상", "결제·뽑기 없음"), "규율 위반"·"실패"·"벌점" 류 1차 카피, 방 온기/규율의 **숫자·점수·퍼센트 노출 금지** (§0.5.10), **"나만 보는 기록"·"비공개 기록"을 핵심 메트릭의 반복 배지로 노출 금지** (기록 비공개는 기본값 — UX_RULES §10.1 / COPY_POLICY §10.1)
...
### 나의 규율 상세 (My Discipline Detail · P0.1 가벼운 추가/삭제 포함)
- 우선순위: P0 (읽기 + P0.1 가벼운 로컬 프로토타입 추가/삭제 / 영속화·편집·템플릿·알림 연동은 P1) / 사용자 상태: 자기 점검
- 목적: 사용자가 정한 사적 규율을 비공개로 점검 (PRD §0.5.9)
- 핵심 컴포넌트: 오늘의 요약 카드("3개 중 2개 지키는 중" + 방 온기 상태 어휘, **숫자 비표시**, PRD §0.5.10), 규율 리스트, 상태 칩(지키는 중 / 이번 주 실천 / 오늘 확인 필요), 규율 다루는 방식 안내, **헤더 "+규율 추가" 버튼**, **행별 "삭제" 버튼**, **추가 바텀시트(타이틀 "새 규율 추가" + 입력 + 카테고리 칩 + 추가하기/취소)**, **삭제 확인 시트("이 규율을 삭제할까요?")** / 주요 CTA: 홈으로 돌아가기 / 보조 CTA: 규율 추가, 규율 삭제
- 표시 데이터: discipline_rule(로컬 in-memory enum + 사용자 입력 라벨), discipline_category(enum: 밤 시간 / 충동 / 검색 / SNS·숏폼 / 수면), discipline_status(enum), room_warmth 상태(enum, 내부 계산은 비공개)
- 금지 카피: "규율 위반", "실패", "벌점", "위반 횟수", "포기", 공개 약속/감시 프레이밍, 펫 처벌 연결, 정량 점수·등급·랭킹 노출(§0.5.10), **삭제 카피로 "이 규율을 포기할까요?" 금지 — "이 규율을 삭제할까요?" 사용** (COPY_POLICY §10.5)
- 수락 기준: 비공개·비처벌·비감시·회복 우선, P0.1 추가/삭제는 in-memory(영속화·서버·인증·DB 없음), Home 요약과 같은 state 공유, 거친 실패 상태 없음, 정량 모델은 상태 어휘로만 번역됨, 비공개 라벨을 헤더 배지로 노출하지 않음(UX_RULES §10.1)
...
- 캐릭터 진화 (REQ-012)
- 캐릭터 꾸미기 (REQ-013)
- 나의 규율 영속 생성/편집 화면 (Discipline Editor, 본격 편집 UI · 저장 · 동기화) — PRD §0.5.9 / P0.1은 in-memory 프로토타입까지만
