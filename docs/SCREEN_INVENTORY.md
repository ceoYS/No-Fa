...
- 핵심 컴포넌트: 절제 타이머(일·시·분·초), 스트릭, 캐릭터, 체크인 진입, 오늘 보호한 자극 카드, **나의 규율 요약 카드(샘플 3개 + 상태 칩, PRD §0.5.9)**, 최근 7일 미니 스트립, (차단 상태 표시는 PoC 범위 내) / 주요 CTA: 오늘 상태 기록하기 / 보조 CTA: 충동 멈추기, 규율 보기
- 표시 데이터: 절제 경과 시간, 현재/최장 스트릭, character_stage, 오늘 체크인 여부, 오늘 보호한 자극 count(이미지/영상 mock count), discipline 상태(enum: 지키는 중 / 이번 주 실천 / 오늘 확인 필요)
- 금지 카피: "참아라", 명령형, 내부 정책 어휘(예: "중립 표기·처벌 없음", "코스메틱·케어 기반 보상", "결제·뽑기 없음"), "규율 위반"·"실패"·"벌점" 류 1차 카피
- 수락 기준: 오늘 할 행동 CTA 명확, **자기통제/충동 보호/회복 앱으로 명료히 읽힘** (PRD §0.5.8), 잔불·펫은 보조 메타포로만 사용
...
### 나의 규율 상세 (My Discipline Detail · 읽기 전용)
- 우선순위: P0 (읽기 전용, 단순 시 포함 / 복잡 시 P1로 이월) / 사용자 상태: 자기 점검
- 목적: 사용자가 정한 사적 규율을 비공개로 점검 (PRD §0.5.9)
- 핵심 컴포넌트: 규율 리스트(샘플), 상태 칩(지키는 중 / 이번 주 실천 / 오늘 확인 필요), 규율 다루는 방식 안내 / 주요 CTA: 홈으로 돌아가기 / 보조 CTA: 없음
- 표시 데이터: discipline_rule(샘플 enum), discipline_status(enum)
- 금지 카피: "규율 위반", "실패", "벌점", "위반 횟수", 공개 약속/감시 프레이밍, 펫 처벌 연결
- 수락 기준: 비공개·비처벌·비감시·회복 우선, 편집 UI 없음(P0), 거친 실패 상태 없음
...
- 캐릭터 진화 (REQ-012)
- 캐릭터 꾸미기 (REQ-013)
- 나의 규율 생성/편집 화면 (Discipline Editor) — PRD §0.5.9
